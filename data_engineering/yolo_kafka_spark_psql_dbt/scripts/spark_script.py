...
# --- Spark and PostgreSQL settings ---
POSTGRES_HOST = os.getenv("POSTGRES_HOST", "highway-postgres-instance")
...
def spark_to_postgres():
    ...
    try:
        ...
        spark = (
            SparkSession.builder.appName("sparkToPostgres").master(spark_master_url)
            # .config(
            #     "spark.jars.packages",
    ...
    def write_to_postgres_batch(batch_df, batch_id):
        ...
        try:
            # 打印 Schema 和一些樣本數據，幫助調試
            logging.info(f"Batch {batch_id}: Schema of DataFrame to write:")
            batch_df.printSchema()
            logging.info(f"Batch {batch_id}: Sample data (up to 5 rows):")
            ...
            logging.error(
                f"Batch {batch_id}: Failed to write to PostgreSQL: {e}", exc_info=True
            )

    query = (
        df.writeStream.foreachBatch(write_to_postgres_batch)
        .outputMode("append")
        .option("checkpointLocation", "/mnt/spark-checkpoints/postgres_highway_traffic")
        .start()
    )
